...
const statusMap: Record<string, { label: string; color: string }> = {
  active: { label: '开放中', color: 'green' },
  closed: { label: '已关闭', color: 'default' },
  maintenance: { label: '维护中', color: 'orange' },
};

const CoolingSpotList = () => {
  const [data, setData] = useState<CoolingSpot[]>([]);
  ...
  const [editingRecord, setEditingRecord] = useState<CoolingSpot | null>(null);
  ...
  const fetchData = async (pageNum = page, pageSizeNum = pageSize, params = searchParams) => {
    setLoading(true);
    try {
      const res: any = await api.get('/cooling-spots', {
        params: { page: pageNum, pageSize: pageSizeNum, ...params },
      });
      setData(res?.list || []);
      setTotal(res?.total || 0);
    } catch (error) {
      console.error('获取纳凉点列表失败:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (record: CoolingSpot) => {
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.delete(`/cooling-spots/${id}`);
  ...
  const handleToggleStatus = async (record: CoolingSpot) => {
    const newStatus = record.status === 'active' ? 'closed' : 'active';
    try {
      if (newStatus === 'closed') {
        await api.put(`/cooling-spots/${record.id}/close`);
      } else {
        await api.put(`/cooling-spots/${record.id}/activate`);
      }
      message.success(newStatus === 'active' ? '已激活' : '已关闭');
      fetchData(page, pageSize, searchParams);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSubmitting(true);
      if (editingRecord) {
        await api.put(`/cooling-spots/${editingRecord.id}`, submitData);
        message.success('编辑成功');
      } else {
        await api.post('/cooling-spots', submitData);
  ...
  const handleExport = async () => {
    try {
      const res: any = await api.get('/cooling-spots/export', { params: searchParams });
      const list = res || [];
      if (list.length === 0) {
        message.warning('没有可导出的数据');
        return;
      }
      const headers = ['名称', '地址', '开放时间', '关闭时间', '容纳人数', '状态'];
      const csvContent = [
        headers.join(','),
        ...list.map((item: any) =>
          [item.name, item.address, item.openTime, item.closeTime, item.capacity, statusMap[item.status]?.label || item.status].join(',')
        ),
      ].join('\n');
      const blob = new Blob(['\ufeff' + csvContent], { type: 'text/csv;charset=utf-8;' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `纳凉点_${new Date().toLocaleDateString()}.csv`;
      link.click();
  ...
  const columns: ColumnsType<CoolingSpot> = [
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Button
            ...
            onClick={() =>
              Modal.confirm({
                title: record.status === 'active' ? '确定要关闭该纳凉点吗?' : '确定要激活该纳凉点吗?',
                okText: '确定',
                cancelText: '取消',
                maskStyle: { background: 'rgba(0,0,0,0.5)' },
                centered: true,
  ...
  return (
    <PageContainer title="纳凉点">
      ...
      <Toolbar onAdd={handleAdd} addText="新增纳凉点" onImport={handleImport} onExport={handleExport} />
      ...
      <Modal
        title={editingRecord ? '编辑纳凉点' : '新增纳凉点'}
        open={modalOpen}
        onOk={handleSubmit}
      ...
      >
        <Form form={form} layout="vertical" preserve={false}>
          ...
          <Form.Item name="capacity" label="容纳人数" rules={[{ required: true, message: '请输入容纳人数' }]}>
            <InputNumber style={{ width: '100%' }} min={0} placeholder="请输入容纳人数" />
          </Form.Item>
...
export default CoolingSpotList;
